...
# Files input #
Sample1_file = "Sample1_Transport_unfiltered.csv"
...
Genes_of_interest_Transporters = {}
...
with open(os.path.join(folder1,Transporter_filtration_list),'r') as read:
    ...
    for line in read:
        ...
        if line[0] in Sample1_genes:
            ...
            if not Sample1_genes[line[0]].startswith('SLC25') and not Sample1_genes[line[0]].startswith('atp5'.upper()) and not Sample1_genes[line[0]].startswith('atp6v'.upper()):
                # If Cellular Component (CC) is empty - Add empty tag
                if line[1] == '':
                    Genes_of_interest_Transporters[line[0]] = 'Empty tag'
                # If Cellular Component (CC) contains plasma membrane tag - add Plasma membrane tag 
                    ...
                    Genes_of_interest_Transporters[line[0]] = 'Plasma membrane tag'
                    ...
                    if not ("lysosome".upper() in line[1].upper() or "endosome membrane".upper() in line[1].upper() or "lysosomal".upper() in line[1].upper()):
                        if not ("mitochondrion".upper() in line[1].upper() or "mitochondrial".upper() in line[1].upper()):
                            if not ("golgi apparatus".upper() in line[1].upper() or "vacuolar".upper() in line[1].upper() or "endoplasmic".upper() in line[1].upper()):
                                Genes_of_interest_Transporters[line[0]] = 'Filtered tag'
                            else:
                                pass
                        else:
                            pass
...
with open(os.path.join(folder3,Filtered_file),'w+') as out:
    ...
    for key in Genes_of_interest_Transporters:
        out.write("{};{};{}\n".format(key,Sample1_genes[key],Genes_of_interest_Transporters[key]))
out.close
